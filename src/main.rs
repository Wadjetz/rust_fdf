...
fn main() {
    ...
    if let Some(directories) = matches.values_of("directory") {
        ...
        let files_index: Vec<FileIndex> = directories_paths
            .par_iter()
            .map(Path::new)
        ...
        let files_index = files_index
            .iter()
            .fold(HashMap::new(), |mut acc, file_index| {
                {
                    let entry = acc.entry(file_index.hash.clone()).or_insert_with(Vec::new);
...
pub fn get_response() -> Result<String, IOError> {
    ...
    stdin.read_line(&mut stdin_buffer).map(|_| stdin_buffer)
}
